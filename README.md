...
This project uses the [xdeploy](https://github.com/pcaversaccio/xdeployer) Hardhat plugin. Check out the documentation for more information on the specifics of the deployments.
...
This project is currently configured via the [hardhat.config.ts](./hardhat.config.ts) as follows:
...
```bash
npx hardhat verify --network goerli --constructor-args arguments.js <YOUR_CONTRACT_ADDRESS>
```

## Foundry

This project repository also includes the [Foundry](https://github.com/foundry-rs/foundry) toolkit.
...
To fix 'foundry update'
1. rm -rf lib
2. install each dependency like this : git submodule add --force --name lib/forge-std https://github.com/foundry-rs/forge-std lib/forge-std
3. forge update works now !